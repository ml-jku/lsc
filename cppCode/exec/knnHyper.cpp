...
bool compareKNNRec(const KNNRec &a, const KNNRec &b) {
  if (a.value > b.value)
    return true;
  return false;
}
...
int main(int argc, char** argv) {
  ...
  std::vector<long> paramKNNTypes;
  for(int i=0; i<cfg.lookup("paramKNNTypes").getLength(); i++)
    paramKNNTypes.push_back(cfg.lookup("paramKNNTypes")[i]);
  long defaultKNN=cfg.lookup("defaultKNN");
  ...
  std::vector<KNNHyperParam> paramComb;
  for(long i=0L; i<paramFeatureTypes.size(); i++) {
    for(long j=0L; j<paramKNNTypes.size(); j++) {
      paramComb.push_back(KNNHyperParam(paramFeatureTypes[i], j));
    }
  }
  std::sort(paramComb.begin(), paramComb.end(), paramSortKNN);
  ...
  printf("Running knn with the following parameters:\n");
  printf("project:              %30s\n", projectName.c_str());
        ...
        {
          {
            ...
            long maxKNN=0L;
            long* paramKNN=new long[paramKNNTypes.size()];
            for(long i=0L; i<paramKNNTypes.size(); i++) {
              paramKNN[i]=paramKNNTypes[i];
              if(paramKNN[i]>maxKNN)
                maxKNN=paramKNN[i];
            }
            ...
            {
              {
                ...
                std::vector<KNNRec**> knnMat;
                std::vector<KNNRec**> knnMatUse;
                ...
                for(long paramInd=featureParamInd[fpi]; paramInd<featureParamInd[fpi+1L]; paramInd++) {
                  ...
                  long knn=paramComb[paramInd].knn;
                  long nrNeighbours=paramKNN[knn];
                  ...
                  if(oldFeatureParamId!=featureParamId) {
                    ...
                    prob=new KNNRec*[nrPred];
                    probStore=new KNNRec[nrPred*nrTrain];
                    
                    #ifdef multiproc
                    #pragma omp parallel for
                    #endif
                    for(long i=0L; i<nrPred; i++) {
                      prob[i]=&(probStore[i*nrTrain]);
                      for(long j=0L; j<nrTrain; j++) {
                        prob[i][j].index=j;
                        prob[i][j].value=simMatrixPred[j][i];
                      }
                      long sortNr=maxKNN;
                      if(maxKNN>nrTrain)
                        sortNr=nrTrain;
                      std::partial_sort(prob[i], prob[i]+sortNr, prob[i]+nrTrain, compareKNNRec);
                    }
                    
                    knnMat.push_back(prob);
                  }
                ...
                #endif
                for(long paramInd=featureParamInd[fpi]; paramInd<featureParamInd[fpi+1L]; paramInd++) {
                  KNNRec** predKNNMat=knnMatUse[paramInd-featureParamInd[fpi]];
                  ...
                  myss << currentPredSavePathname << "dbgClustProb_" << clusterIndEval << "_" << clusterIndHyper << "_" << featureParam.id;

                  string outFilenameProb=myss.str();
                  FILE* outFileProb=fopen(outFilenameProb.c_str(), "w");
                  double* predVec=(double*)malloc(sizeof(double)*nrPred);                  
                  for(long i=0L; i<nrPred; i++) {
                    double avg=0.0;
                    for(long j=0L; j<end; j++)
                      avg=avg+labelTrain[predKNNMat[i][j].index];
                    avg=avg/((double)(end));
                    predVec[i]=avg;
                    fprintf(outFileProb, "%.10lf\n", avg);
                  }
            ...
            for(long k=featureParamInd[fpi]; k<featureParamInd[fpi+1L]; k++) {
              fprintf(outFileHyperTrainAUC, "featureParam=%ld/knn=%ld", paramComb[k].fkc.id, paramKNN[paramComb[k].knn]);
              for(long i=0L; i<nrClusters; i++) {
                //if(i!=clusterIndEval) fprintf(outFileHyperTrainAUC, ",%5.10lf", aucTrain[i][k]);
                //else 
                fprintf(outFileHyperTrainAUC, ",NA");
              }
            ...
            for(long k=featureParamInd[fpi]; k<featureParamInd[fpi+1L]; k++) {
              fprintf(outFileHyperPredAUC, "featureParam=%ld/knn=%ld", paramComb[k].fkc.id, paramKNN[paramComb[k].knn]);
              for(long i=0L; i<nrClusters; i++) {
            ...
            fprintf(outFileHyperPredAUC, "featureParam=%ld/knn=%ld", paramComb[aucSelIndex].fkc.id, paramKNN[paramComb[aucSelIndex].knn]);
            for(long i=0L; i<nrClusters; i++) {
            ...
            KNNRec** prob=new KNNRec*[nrPred];
            KNNRec* probStore=new KNNRec[nrPred*nrTrain];
  ...
  printf("knnHyper terminated successfully!\n");
